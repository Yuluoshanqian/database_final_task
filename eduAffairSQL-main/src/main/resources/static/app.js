...
const studentSelectedCreditRoutes = new Set([
  "courseSelect",
  "selectedCourses",
  "grades",
  "noticeManage",
...
function weekTypeOptions(selected = "all") {
  return Object.entries(weekTypeText)
    .map(
      ([value, label]) =>
...
function courseBadge(status) {
  ...
  const label =
    status === "disabled" ? "弃用" : status === "enabled" ? "启用" : status;
...
function collectOfferingPayload(form) {
  ...
  data.times = Array.from(form.querySelectorAll("[data-time-segment]")).map(
    (segment) => ({
      classroomId: Number(
        segment.querySelector('[name="timeClassroomId"]').value,
      ),
      dayOfWeek: Number(segment.querySelector('[name="timeDayOfWeek"]').value),
      startSection: Number(
        segment.querySelector('[name="timeStartSection"]').value,
      ),
      endSection: Number(
        segment.querySelector('[name="timeEndSection"]').value,
      ),
...
async function loadRoute(force = false) {
  ...
  if (route === "dashboard") {
    ...
    if (role === "teacher") {
      ...
      state.routeData.teacherGradeCourses = await api(
        "/api/teacher/grade-courses",
      );
  ...
  if (role === "admin") {
    if (route === "teacherManage") {
      state.routeData.teachers = await api(
        `/api/admin/teachers?keyword=${encodeURIComponent(state.filters.teacherKeyword || "")}`,
      );
    } else if (route === "studentManage") {
      state.routeData.students = await api(
        `/api/admin/students?keyword=${encodeURIComponent(state.filters.studentKeyword || "")}`,
      );
    } else if (route === "courseCatalog") {
      state.routeData.adminCourses = await api(
        `/api/admin/courses?keyword=${encodeURIComponent(state.filters.adminCourseKeyword || "")}`,
      );
    } else if (route === "courseManage") {
      const semesterId = ensureAdminSemesterSelection("offeringSemesterId");
      const params = new URLSearchParams({
        keyword: state.filters.offeringKeyword || "",
      });
      if (semesterId) params.set("semesterId", semesterId);
      state.routeData.offerings = await api(
        `/api/admin/offerings?${params.toString()}`,
      );
    } else if (route === "auditLogs") {
      state.routeData.auditLogs = await api(
        `/api/admin/logs?page=${encodeURIComponent(state.logPage)}&pageSize=${encodeURIComponent(state.logPageSize)}`,
      );
    } else if (route === "backupManage") {
      state.routeData.backups = await api(
        `/api/admin/backups?page=${encodeURIComponent(state.backupPage)}&pageSize=${encodeURIComponent(state.backupPageSize)}`,
      );
  ...
  if (role === "teacher") {
    ...
    if (route === "gradeEntry") {
      ...
      if (
        !gradable.some(
          (item) => String(item.id) === String(state.selected.gradeOfferingId),
        )
      ) {
        state.selected.gradeOfferingId = String(gradable[0].id);
      }
      if (state.selected.gradeOfferingId) {
        state.routeData.gradeRoster = await api(
          `/api/teacher/grade-roster?offeringId=${encodeURIComponent(state.selected.gradeOfferingId)}`,
        );
      } else {
        state.routeData.gradeRoster = [];
      }
  ...
  if (role === "student") {
    if (studentSelectedCreditRoutes.has(route)) {
      const semesterId =
        route === "selectedCourses"
          ? ensureCatalogSemesterSelection("scheduleSemesterId")
          : "";
    ...
    if (route === "courseSelect") {
      state.routeData.studentOfferings = await api(
        `/api/student/offerings?keyword=${encodeURIComponent(state.filters.courseKeyword || "")}`,
      );
...
function termStrip() {
  ...
  const role = state.user?.role;
  const creditLabel =
    state.route === "selectedCourses" ? "所选学期已选学分" : "本学期已选学分";
  const selectedCredits =
    state.user?.role === "student" &&
    studentSelectedCreditRoutes.has(state.route)
...
function gradeDistribution(distribution = {}) {
  ...
  const total =
    rows.reduce((sum, [, value]) => sum + Number(value || 0), 0) || 1;
  return `
    <div class="bar-chart">
      ${rows
        .map(([label, value]) => {
...
function renderProfile() {
  ...
  if (user.role === "teacher") {
    rows.push(
      ["工号", profile.teacherNo],
      ["院系", profile.departmentName],
      ["职称", profile.title],
    );
    ...
    rows.push(
      ...
      ["院系", profile.departmentName],
      ["专业", profile.majorName],
      ["入学年份", profile.admissionYear],
    );
  } else {
...
function renderEditSemesterModal(semesterId) {
  const catalog = state.adminCatalog || {};
  const current =
    (catalog.semesters || []).find(
      (term) => String(term.id) === String(semesterId),
...
function renderCourseCatalog() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(courses.length / state.adminCoursePageSize),
  );
          ...
          <tr>
            ...
            <td><div class="row-actions">${
              row.status === "enabled"
                ...
                : `<button class="btn btn-primary" data-action="course-enable" data-id="${row.id}">启用</button>`
            }</div></td>
          </tr>
        `,
          )
...
function ensureAdminSemesterSelection(key) {
  ...
  const exists = semesters.some(
    (semester) => String(semester.id) === String(currentValue),
  );
  if (!exists) {
...
function selectedAdminSemester(key) {
  ...
  return (state.adminCatalog?.semesters || []).find(
    (semester) => String(semester.id) === selectedId,
  );
}
...
function selectedCatalogSemester(key) {
  ...
  return (state.catalog?.semesters || []).find(
    (semester) => String(semester.id) === selectedId,
  );
}
...
function renderCourseManage() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(offerings.length / state.offeringPageSize),
  );
...
function renderAuditLogs() {
  const data = state.routeData.auditLogs || {
    rows: [],
    page: 1,
    totalPages: 1,
    total: 0,
...
function renderBackupManage() {
  const data = state.routeData.backups || {
    rows: [],
    page: 1,
    totalPages: 1,
    total: 0,
...
function renderEditOfferingModal(offeringId) {
  ...
  if (!offering) return "";
  const examPct =
    offering.examRatio != null
      ? Math.round(Number(offering.examRatio) * 100)
      : "";
      ...
      <div class="modal roster-modal">
        ...
        <div class="modal-body">
          <div class="table-wrap"><table>
            ...
            <tbody>${state.routeData.rosterData
              .map(
                (row) => `
              <tr>
...
function renderStudents() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(rows.length / state.studentPageSize),
  );
...
function renderTeachers() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(rows.length / state.teacherPageSize),
  );
...
function renderTeacherRosterModal(offeringId) {
  ...
  const title = course
    ? `${text(course.courseCode)} ${text(course.courseName)}`
    : "学生名单";
  return `
    <div class="modal-backdrop">
      <div class="modal">
...
function courseCard(row) {
  ...
  const selectionOpen = Boolean(
    state.routeData.studentOfferings?.selectionOpen,
  );
  const disabled =
    !selectionOpen || row.status !== "selecting" || passedBefore || full;
...
function scheduleGrid(rows, options = {}) {
  ...
  const entries = rows.flatMap((row) =>
    offeringTimes(row).map((time) => ({ ...row, ...time })),
  );
  const cells = [
    '<div class="schedule-head">节次</div>',
    ...dayNames
      .slice(1)
      .map((day) => `<div class="schedule-head">${day}</div>`),
  ];
  slots.forEach(([start, end]) => {
    ...
    for (let day = 1; day <= 7; day += 1) {
      const courses = entries.filter(
        (row) =>
          Number(row.dayOfWeek) === day &&
          Number(row.startSection) <= start &&
          Number(row.endSection) >= end,
      );
      cells.push(
        courses.length
          ? `<div class="schedule-course">${courses
              .map((course) => {
                const teacher =
                  options.showTeacher === false
                    ? ""
...
function renderGrades() {
  ...
  const rows =
    selected === "all"
      ? allRows
      : allRows.filter((row) => String(row.semesterId) === String(selected));
  return `
    <div class="page-grid">
      <section class="panel">
...
function uniqueSemesters(rows) {
  ...
  rows.forEach((row) => {
    if (!map.has(row.semesterId))
      map.set(row.semesterId, { id: row.semesterId, name: row.semesterName });
  });
...
function gpaTrend(rows) {
  ...
  rows.forEach((row, index) => {
    ...
    if (!map.has(key))
      map.set(key, {
        ...
        credits: 0,
        points: 0,
        order: Number(row.semesterId || 0) || -index,
      });
  ...
  const trend = Array.from(map.values())
    ...
    .map((item) => ({
      name: item.name,
      gpa:
        Math.round((item.credits ? item.points / item.credits : 0) * 100) / 100,
    }));
  const width = 760;
  const height = 240;
  const left = 62;
  ...
  const effectiveWidth =
    trend.length <= 1 ? 0 : Math.min(plotWidth, 170 * (trend.length - 1));
  const startX = left + (plotWidth - effectiveWidth) / 2;
  const points = trend.map((item, index) => {
    const x =
      trend.length === 1
        ? left + plotWidth / 2
        : startX + (effectiveWidth / (trend.length - 1)) * index;
  ...
  return rows
    .map((row) => {
      ...
      const sel =
        ...
        String(selectedValue) === String(row[valueKey])
          ? "selected"
          : "";
...
document.addEventListener("click", async (event) => {
  ...
  try {
    ...
    if (action === "navigate") {
      ...
      await api(`/api/admin/notices/${target.dataset.id}`, {
        method: "DELETE",
      });
      await refresh("通知已删除");
      ...
      if (
        !confirm(
          "确定要弃用该课程吗？弃用后不能再新建该课程的课程班，历史课程班不受影响。",
        )
      )
        return;
      await api(`/api/admin/courses/${target.dataset.id}/disable`, {
        method: "POST",
      });
      await refresh("课程已弃用");
    } else if (action === "course-enable") {
      await api(`/api/admin/courses/${target.dataset.id}/enable`, {
        method: "POST",
      });
      await refresh("课程已启用");
      ...
      if (
        !confirm(
          "确定要删除该课程班吗？无成绩且学期未结束时，系统会标记课程班为已删除；已有选课会统一退选。",
        )
      )
        return;
      await api(`/api/admin/offerings/${target.dataset.id}`, {
        method: "DELETE",
      });
      await refresh("课程班已删除");
    } else if (action === "course-roster") {
      state.routeData.rosterData = await api(
        `/api/admin/offerings/${target.dataset.id}/roster`,
      );
      ...
      if (
        !confirm("确定要将该学生从该课程班删除吗？该操作会退掉该学生的这门课。")
      )
        return;
      ...
      await loadRoute(true);
      state.routeData.rosterData = await api(
        `/api/admin/offerings/${offeringId}/roster`,
      );
      state.modal = renderCourseRosterModal(offeringId);
      renderShell();
      toast("学生已退课");
    } else if (action === "teacher-offerings") {
      const semesterId = ensureAdminSemesterSelection(
        "teacherOfferingSemesterId",
      );
      const query = semesterId
        ? `?semesterId=${encodeURIComponent(semesterId)}`
        : "";
      state.routeData.teacherCourseData = await api(
        `/api/admin/teachers/${target.dataset.id}/offerings${query}`,
      );
      state.modal = renderTeacherOfferingsModal(target.dataset.id);
      renderShell();
    } else if (action === "student-enrollments") {
      const semesterId = ensureAdminSemesterSelection(
        "studentEnrollmentSemesterId",
      );
      const query = semesterId
        ? `?semesterId=${encodeURIComponent(semesterId)}`
        : "";
      state.routeData.studentCourseData = await api(
        `/api/admin/students/${target.dataset.id}/enrollments${query}`,
      );
      ...
      await api(`/api/admin/semesters/${target.dataset.id}/selection/start`, {
        method: "POST",
      });
      await refresh("选课已开放");
    } else if (action === "semester-selection-stop") {
      if (!confirm("确定要结束该学期选课吗？关闭后学生不能继续选课或退课。"))
        return;
      await api(`/api/admin/semesters/${target.dataset.id}/selection/stop`, {
        method: "POST",
      });
      await refresh("选课已关闭");
    } else if (action === "semester-grading-start") {
      await api(`/api/admin/semesters/${target.dataset.id}/grading/start`, {
        method: "POST",
      });
      await refresh("登分已开放");
    } else if (action === "semester-grading-stop") {
      if (!confirm("确定要结束该学期登分吗？关闭后教师不能继续保存成绩。"))
        return;
      await api(`/api/admin/semesters/${target.dataset.id}/grading/stop`, {
        method: "POST",
      });
      await refresh("登分已关闭");
      ...
      const list = target
        .closest("form")
        ?.querySelector("[data-offering-times]");
      ...
      if (
        !confirm(
          "确定要弃用该学生吗？弃用后该学生不能登录，历史选课和成绩不受影响。",
        )
      )
        return;
      await api(`/api/admin/students/${target.dataset.id}/disable`, {
        method: "POST",
      });
      await refresh("学生已弃用");
    } else if (action === "student-enable") {
      await api(`/api/admin/students/${target.dataset.id}/enable`, {
        method: "POST",
      });
      await refresh("学生已启用");
      ...
      if (
        !confirm(
          "确定要弃用该教师吗？弃用后该教师不能登录，历史授课安排不受影响。",
        )
      )
        return;
      await api(`/api/admin/teachers/${target.dataset.id}/disable`, {
        method: "POST",
      });
      await refresh("教师已弃用");
    } else if (action === "teacher-enable") {
      await api(`/api/admin/teachers/${target.dataset.id}/enable`, {
        method: "POST",
      });
      await refresh("教师已启用");
      ...
      await api("/api/student/select", {
        method: "POST",
        body: JSON.stringify({ offeringId: Number(target.dataset.id) }),
      });
      await refresh("选课成功");
    } else if (action === "student-drop") {
      try {
        await api("/api/student/drop", {
          method: "POST",
          body: JSON.stringify({ enrollmentId: Number(target.dataset.id) }),
        });
        await refresh("退课成功");
      ...
      state.routeData.teacherRosterData = await api(
        `/api/teacher/roster?offeringId=${encodeURIComponent(target.dataset.id)}`,
      );
  ...
  try {
    if (action === "grade-course-select") {
      ...
      const query = target.value
        ? `?semesterId=${encodeURIComponent(target.value)}`
        : "";
      state.routeData.teacherCourseData = await api(
        `/api/admin/teachers/${target.dataset.teacherId}/offerings${query}`,
      );
      ...
      const query = target.value
        ...
        : "";
      state.routeData.studentCourseData = await api(
        `/api/admin/students/${target.dataset.studentId}/enrollments${query}`,
      );
...
document.addEventListener("submit", async (event) => {
  ...
  try {
    if (name === "login") {
      ...
      await api(`/api/admin/semesters/${form.dataset.semesterId}`, {
        method: "PUT",
        body: JSON.stringify(data),
      });
      state.modal = "";
      ...
      await api("/api/admin/courses", {
        method: "POST",
        body: JSON.stringify(
          asNumberFields(formObject(form), ["departmentId", "credit"]),
        ),
      });
      state.modal = "";
      await refresh("课程已添加");
      ...
      const data = collectOfferingPayload(form);
      await api(`/api/admin/offerings/${offeringId}`, {
        method: "PUT",
        body: JSON.stringify(data),
      });
      state.modal = "";
      ...
      await api("/api/admin/students", {
        method: "POST",
        body: JSON.stringify(
          asNumberFields(formObject(form), ["majorId", "admissionYear"]),
        ),
      });
      state.modal = "";
      await refresh("学生已添加");
      ...
      await api("/api/admin/teachers", {
        method: "POST",
        body: JSON.stringify(
          asNumberFields(formObject(form), ["departmentId"]),
        ),
      });
      state.modal = "";
      await refresh("教师已添加");
      ...
      if (noticeId) {
        await api(`/api/admin/notices/${noticeId}`, {
          method: "PUT",
...
async function saveGrade(enrollmentId) {
  const row = document.querySelector(
    `tr[data-enrollment-id="${CSS.escape(String(enrollmentId))}"]`,
  );
...
function metrics(items, className = "") {
  const rows = items.length
    ...
    : [
        {
          label: "当前学期",
          value: state.catalog?.currentSemester?.name || "暂无",
        },
        {
          label: "选课状态",
          value: friendlyStatus(state.catalog?.currentSemester?.status),
        },
      ];
  return `
...
function networkMeter(metric = {}) {
  ...
  const activeBars =
    status === "online"
      ? Math.max(2, Math.min(4, active || 4))
      : status === "unknown"
        ? 1
        : 0;
  return `
...
function currentTeacherCourses(courses) {
  ...
  if (currentSemesterId) {
    return courses.filter(
      (course) => String(course.semesterId) === String(currentSemesterId),
    );
  }
...
function completedGradeCourseCount(courses) {
  return courses.filter(
    (course) =>
      Number(course.gradedCount || 0) >= Number(course.studentCount || 0),
  ).length;
}
...
function noticeList(notices, options = {}) {
  const rows = Array.isArray(notices)
    ? notices.slice(0, options.limit || notices.length)
    : [];
  if (!rows.length) return empty("暂无通知");
  return `<div class="list-stack notice-list">${rows
    .map(
      (notice) => `
...
function noticeFields(notice = {}) {
  ...
  return `
    ...
    <label class="field"><span>对象</span><select name="audience">
      ${Object.entries(audienceText)
        .map(
          ([value, label]) =>
            `<option value="${value}" ${selectedAudience === value ? "selected" : ""}>${label}</option>`,
        )
        .join("")}
    </select></label>
...
function noticeModal() {
  const notices =
    state.catalog?.notices || state.routeData.dashboard?.notices || [];
  return `
    <div class="modal-backdrop">
...
function renderNoticeManage() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(notices.length / state.noticePageSize),
  );
...
function renderGradeEntry() {
  ...
  const selectedCourse = courses.find(
    (course) => String(course.id) === String(state.selected.gradeOfferingId),
  );
  return `
    <div class="page-grid">
      <section class="panel">
